...
@test "the scratch env file does not survive, on either path" {
  ...
  RNW_BUILD_ENV='{"API_KEY":"x"}' publish
  [ "$status" -ne 0 ] || fail "accepted a credential key: $output"
  [ ! -f "$RUNNER_TEMP/rnw-build-env.env" ] || fail "left a scratch file behind after a rejection"
}
...
# I3: build-env is published before fingerprint.sh runs, so RNW_FP_* would hand
...
@test "a key owned by the family or the runner is refused" {
  for k in RNW_FP_IOS RNW_ASSETS_DIR GITHUB_REPOSITORY RUNNER_TEMP ACTIONS_STEP_DEBUG PATH HOME LD_PRELOAD DYLD_INSERT_LIBRARIES NODE_OPTIONS; do
    RNW_BUILD_ENV="{\"$k\":\"x\"}" publish
...
@test "GITHUB_TOKEN is refused as a credential, not as a reserved name" {
  RNW_BUILD_ENV='{"GITHUB_TOKEN":"x"}' publish
