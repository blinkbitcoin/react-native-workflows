...
setup() {
  ...
  # RNW_CONSUMER_ROOT (that one points the *tests* at a checkout, not the
...
#!/usr/bin/env bash
printf '%s\n' "pnpm $*" >> "$RNW_TEST_LOG"
exit "${RNW_TEST_PNPM_EXIT:-0}"
SH
  chmod +x "$STUB/pnpm"
  export RNW_TEST_LOG="$BATS_TEST_TMPDIR/calls.log"
  : > "$RNW_TEST_LOG"
}
...
@test "the consumer's own script wins when it ships one" {
  ...
  grep -q '^pnpm run deps:audit$' "$RNW_TEST_LOG" || fail "the consumer's script did not run: $(cat "$RNW_TEST_LOG")"
  ! grep -q 'fallback ran' "$RNW_TEST_LOG" || fail "the fallback ran as well: $(cat "$RNW_TEST_LOG")"
}
...
@test "the fallback runs when the consumer ships no such script" {
  ...
  run_it 'deps:audit'
  [ "$status" -eq 0 ] || fail "exited $status: $output"
  grep -q 'fallback ran' "$RNW_TEST_LOG" || fail "the fallback did not run: $(cat "$RNW_TEST_LOG")"
  ! grep -q 'pnpm run' "$RNW_TEST_LOG" || fail "it ran a consumer script that does not exist"
}
...
@test "a failing consumer script fails the step" {
  write_package_json 'deps:audit'
  write_fallback
  RNW_TEST_PNPM_EXIT=3 run_it 'deps:audit'
